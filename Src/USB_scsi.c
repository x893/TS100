...
uint8_t Page00_Inquiry_Data[]	= { 0, 0, 0, 0, 0};
uint8_t Mode_Sense6_data[]		= { 0x03, 0x00, 0x00, 0x00};
uint8_t Mode_Sense10_data[]  	= { 0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
uint8_t Scsi_Sense_Data[]		= { 0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0A, 0x00,
									0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
									};
uint8_t ReadCapacity10_Data[]	= { 0, 0, 0, 0, 0, 0, 0, 0};
uint8_t ReadFormatCapacity[]	= { 0, 0, 0, 8, 0, 0, 0, 0, 2, 0, 0, 0};

#ifdef DFU_MODE
	uint8_t Disk_Inquiry_Str[] = {
							0x00, 0x80, 0x02, 0x02, 36-4, 0x00, 0x00, 0x00,
							'V',  'i',  'r',  't',  'u',  'a',  'l',  ' ',
							'D',  'F',  'U',  ' ',  'D',  'i',  's',  'k',
							' ',  ' ',  ' ',  ' ',  ' ',  ' ',  ' ',  ' ',
							' ',  ' ',  ' ',  ' ',
...
void SCSI_Inquiry_Cmd(void)
{
	...
	{
		Inquiry_Data = (uint8_t*)Disk_Inquiry_Str;
...
{
	ReadFormatCapacity[ 4] = (uint8_t)(SECTOR_SIZE >> 24);
	ReadFormatCapacity[ 5] = (uint8_t)(SECTOR_SIZE >> 16);
	ReadFormatCapacity[ 6] = (uint8_t)(SECTOR_SIZE >>  8);
	ReadFormatCapacity[ 7] = (uint8_t)(SECTOR_SIZE);

	ReadFormatCapacity[ 9] = (uint8_t)(SECTOR_SIZE >> 16);
	ReadFormatCapacity[10] = (uint8_t)(SECTOR_SIZE >>  8);
	ReadFormatCapacity[11] = (uint8_t)(SECTOR_SIZE);
...
{
	uint32_t cnt = SECTOR_CNT - 1;
	ReadCapacity10_Data[0] = (uint8_t)(cnt >> 24);
	ReadCapacity10_Data[1] = (uint8_t)(cnt >> 16);
	ReadCapacity10_Data[2] = (uint8_t)(cnt >>  8);
	ReadCapacity10_Data[3] = (uint8_t)(cnt);

	ReadCapacity10_Data[4] = (uint8_t)(SECTOR_SIZE >>  24);
	ReadCapacity10_Data[5] = (uint8_t)(SECTOR_SIZE >>  16);
	ReadCapacity10_Data[6] = (uint8_t)(SECTOR_SIZE >>  8);
	ReadCapacity10_Data[7] = (uint8_t)(SECTOR_SIZE);
...
*******************************************************************************/
uint8_t SCSI_Address_Management(uint8_t Cmd , u32 LBA , u32 BlockNbr)
{
